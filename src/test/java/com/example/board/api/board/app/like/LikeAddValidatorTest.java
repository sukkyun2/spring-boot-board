...
class LikeAddValidatorTest {
    ...
    @Test
    @DisplayName("좋아요 클릭시 사용자 정보가 필요하다")
    void given_user_id_null_then_throw_exception() {
        LikeAddRequest givenReq = new LikeAddRequest(null, 1, LikeType.LIKE);

        ValidationException ex = assertThrows(ValidationException.class, () -> validator.validate(givenReq));
        assertThat(ex).hasMessage(ERROR_MSG_USER_ID_IS_NULL);
    ...
    void given_board_seq_null_then_throw_exception() {
        LikeAddRequest givenReq = new LikeAddRequest(1, null, LikeType.LIKE);
    ...
    void given_not_exists_board_then_dont_add_like(){
        LikeAddRequest givenReq = new LikeAddRequest(1, 2, LikeType.LIKE);
    ...
    void given_user_added_like_then_dont_add_like(){
        ...
        given(likeRepository.existsById(new LikeId(1, 2))).willReturn(true);
